...
class DatabaseInterface:
    ...
    def __init__(self):
        ...
        with open(f"{script_path}/resources/dois.json", "r", encoding="utf-8") as dois_file:
            self.extra_dois = json.load(dois_file)
        with open(
            f"{script_path}/resources/contributors_organizations.json", "r", encoding="utf-8"
        ...
        with open(
            f"{script_path}/resources/public_collections.json", "r", encoding="utf-8"
        ) as coll_file:
    ...
    def __get_file_size_for_catalog(self, url, recurse=None):
        ...
        if recurse is None:
            no_recurse = (
                "/IDRA/",
                "/darelux/",
                "/zandmotor/meteohydro/xband/catalog",
                "/CF_Drinking_water/",
            )
        ...
        if not recurse:
            logging.debug(
                "Catalog %s is not recursed, subcatalogs are linked to other datasets.", url
            )
        elif not references:
    ...
    def __load_resource_file(self, filename):
        ...
        data = None
        try:
            with open(f"{script_path}/resources/{filename}", "r", encoding="utf-8") as data_file:
    ...
    def insert_account(self, record):
        ...
        with self.lock_for_inserts:
            ...
            rdf.add(
                self.store, uri, rdf.DJHT["active"], value_or(record, "active", False), XSD.boolean
            )
            ...
            rdf.add(
                ...
                uri,
                rdf.DJHT["first_name"],
                value_or(record, "first_name", None),
                XSD.string,
            )
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["last_name"],
                value_or(record, "last_name", None),
                XSD.string,
            )
            rdf.add(
                self.store, uri, rdf.DJHT["institution_user_id"], institution_user_id, XSD.string
            )
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["institution_id"],
                value_or(record, "institution_id", None),
            )
            rdf.add(self.store, uri, rdf.DJHT["group_id"], value_or(record, "group_id", None))
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["pending_quota_request"],
                value_or(record, "pending_quota_request", None),
            )
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["used_quota_public"],
                value_or(record, "used_quota_public", None),
            )
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["used_quota_private"],
                value_or(record, "used_quota_private", None),
            )
            rdf.add(self.store, uri, rdf.DJHT["used_quota"], value_or(record, "used_quota", None))
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["maximum_file_size"],
                value_or(record, "maximum_file_size", None),
            )
            rdf.add(self.store, uri, rdf.DJHT["quota"], value_or(record, "quota", None))
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["modified_date"],
                value_or_none(record, "modified_date"),
                XSD.dateTime,
            )
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["created_date"],
                value_or_none(record, "created_date"),
                XSD.dateTime,
            )
    ...
    def insert_author(self, record):
        ...
        rdf.add(
            self.store, uri, rdf.DJHT["group_id"], value_or(record, "group_id", None), XSD.integer
        )
        rdf.add(
            self.store,
            uri,
        ...
        rdf.add(
            self.store, uri, rdf.DJHT["last_name"], value_or(record, "last_name", None), XSD.string
        )
        rdf.add(
            self.store, uri, rdf.DJHT["full_name"], value_or(record, "full_name", None), XSD.string
        )
        rdf.add(
            self.store, uri, rdf.DJHT["job_title"], value_or(record, "job_title", None), XSD.string
        )
        rdf.add(
            self.store, uri, rdf.DJHT["url_name"], value_or(record, "url_name", None), XSD.string
        )
        rdf.add(
            self.store, uri, rdf.DJHT["orcid_id"], value_or(record, "orcid_id", None), XSD.string
        )
    ...
    def insert_timeline(self, uri, record):
        ...
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["revision_date"],
            value_or_none(record, "revision"),
            XSD.dateTime,
        )
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["publisher_publication_date"],
            value_or_none(record, "publisherPublication"),
            XSD.dateTime,
        )
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["publisher_acceptance_date"],
            value_or_none(record, "publisherAcceptance"),
            XSD.dateTime,
        )
        rdf.add(
            self.store, uri, rdf.DJHT["posted_date"], value_or_none(record, "posted"), XSD.dateTime
        )
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["submission_date"],
            value_or_none(record, "submission"),
            XSD.dateTime,
        )
    ...
    def insert_category(self, record):
        ...
        rdf.add(
            self.store, uri, rdf.DJHT["parent_id"], value_or(record, "parent_id", None), XSD.integer
        )
        rdf.add(
            self.store, uri, rdf.DJHT["source_id"], value_or(record, "source_id", None), XSD.integer
        )
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["taxonomy_id"],
            value_or(record, "taxonomy_id", None),
            XSD.integer,
        )
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["classification_code"],
            value_or(record, "classification_code", None),
            XSD.string,
        )
    ...
    def insert_record_list(self, uri, records, name, insert_procedure):
        ...
        if records:
            ...
            for index, item in enumerate(records):
                ...
                self.store.add(
                    (blank_node, rdf.DJHT["index"], Literal(index, datatype=XSD.integer))
                )
    ...
    def insert_custom_field(self, uri, field):
        ...
        subjects = self.store.subjects(
            (None, RDF.type, rdf.DJHT["CustomField"]),
        ...
        if field_uri is None:
            ...
            rdf.add(
                ...
                field_uri,
                rdf.DJHT["max_length"],
                value_or_none(validations, "max_length"),
            )
            rdf.add(
                self.store,
                field_uri,
                rdf.DJHT["min_length"],
                value_or_none(validations, "min_length"),
            )
            rdf.add(
                self.store,
                field_uri,
                rdf.DJHT["is_mandatory"],
                value_or_none(validations, "is_mandatory"),
                XSD.boolean,
            )
            rdf.add(
                self.store,
                field_uri,
                rdf.DJHT["placeholder"],
                value_or_none(validations, "placeholder"),
                XSD.string,
            )
            rdf.add(
                self.store,
                field_uri,
                rdf.DJHT["is_multiple"],
                value_or_none(validations, "is_multiple"),
                XSD.boolean,
            )
    ...
    def fix_doi(self, record, item_id, version, item_type):
        ...
        extra_dois = [
            extra["doi"]
            for extra in self.extra_dois[item_type]
            if extra["id"] == item_id and extra["version"] == version
        ]
        if extra_dois:
    ...
    def handle_custom_fields(self, record, uri, item_id, version, item_type):
        ...
        for field in value_or(record, "custom_fields", []):
            ...
            if field["name"] == "Data Link":
                urls = field["value"]
                urls = [
                    url.replace("https://opendap.tudelft.nl/", "https://opendap.4tu.nl/")
                    for url in urls
                ]
                field["value"] = urls
                ...
                if data_link_size:
                    rdf.add(
                        self.store, uri, rdf.DJHT["data_link_size"], data_link_size, XSD.integer
                    )
    ...
    def insert_collection(self, record, account_id, account_uri):
        ...
        with self.lock_for_inserts:
            ...
            self.store.add(
                (uri, rdf.DJHT["collection_id"], Literal(collection_id, datatype=XSD.integer))
            )
            ...
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["published_date"],
                value_or_none(record, "published_date"),
                XSD.dateTime,
            )
            rdf.add(
                self.store,
            ...
            rdf.add(
                self.store, uri, rdf.DJHT["citation"], value_or_none(record, "citation"), XSD.string
            )
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["group_id"],
                value_or_none(record, "group_id"),
                XSD.integer,
            )
            ...
            rdf.add(
                ...
                uri,
                rdf.DJHT["institution_id"],
                value_or_none(record, "institution_id"),
                XSD.integer,
            )
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["description"],
                value_or_none(record, "description"),
                XSD.string,
            )
            rdf.add(
            ...
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["resource_doi"],
                value_or_none(record, "resource_doi"),
                XSD.string,
            )
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["resource_title"],
                value_or_none(record, "resource_title"),
                XSD.string,
            )
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["resource_version"],
                value_or_none(record, "resource_version"),
                XSD.string,
            )
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["resource_link"],
                value_or_none(record, "resource_link"),
                XSD.string,
            )
            rdf.add(
                self.store, uri, rdf.DJHT["handle"], value_or_none(record, "handle"), XSD.string
            )
            rdf.add(self.store, uri, rdf.DJHT["is_public"], is_public, XSD.boolean)
            ...
            tags = [
                tag for tag in value_or(record, "tags", []) if not tag.startswith("Collection: ")
            ]
            ...
            datasets = value_or(
                record,
                "datasets",
                value_or(self.public_collection_datasets, f"{collection_id}_{version}", []),
            )
            if datasets:
            ...
            container = self.container_uri(collection_id, "collection", account_id, account_uri)
            ...
            if is_latest:
                ...
                rdf.add(
                    self.store,
                    container,
                    rdf.DJHT["first_online_date"],
                    value_or_none(timeline, "firstOnline"),
                    XSD.dateTime,
                )
    ...
    def insert_funding(self, record):
        ...
        rdf.add(self.store, uri, rdf.DJHT["title"], value_or_none(record, "title"), XSD.string)
        rdf.add(
            self.store, uri, rdf.DJHT["grant_code"], value_or_none(record, "grant_code"), XSD.string
        )
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["funder_name"],
            value_or_none(record, "funder_name"),
            XSD.string,
        )
        rdf.add(self.store, uri, rdf.DJHT["url"], value_or_none(record, "url"), XSD.string)
    ...
    def insert_private_link(self, record):
        ...
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["expires_date"],
            value_or_none(record, "expires_date"),
            XSD.dateTime,
        )
        rdf.add(self.store, uri, rdf.DJHT["is_active"], is_active, XSD.boolean)
    ...
    def insert_license(self, uri, record):
        ...
        if (license_uri, RDF.type, rdf.DJHT["License"]) not in self.store:
            ...
            self.store.add(
                (license_uri, rdf.DJHT["name"], Literal(license_name, datatype=XSD.string))
            )
            self.store.add((license_uri, rdf.DJHT["id"], Literal(license_id, datatype=XSD.integer)))

            if license_spdx is not None:
                self.store.add((license_uri, rdf.DJHT["spdx"], URIRef(license_spdx)))
    ...
    def insert_totals_statistics(self, record, uri):
        ...
        rdf.add(
            self.store, uri, rdf.DJHT["total_views"], value_or_none(record, "views"), XSD.integer
        )
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["total_downloads"],
            value_or_none(record, "downloads"),
            XSD.integer,
        )
        rdf.add(
            self.store, uri, rdf.DJHT["total_shares"], value_or_none(record, "shares"), XSD.integer
        )
        rdf.add(
            self.store, uri, rdf.DJHT["total_cites"], value_or_none(record, "cites"), XSD.integer
        )
    ...
    def insert_file(self, record):
        ...
        if record["download_url"].startswith("https://opendap.tudelft.nl/thredds"):
            record["download_url"] = record["download_url"].replace(
                "opendap.tudelft.nl", "opendap.4tu.nl"
            )

        if (
            record["download_url"].startswith("https://opendap.4tu.nl/thredds")
            and record["size"] == 0
        ):
        ...
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["download_url"],
            value_or_none(record, "download_url"),
            XSD.string,
        )
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["supplied_md5"],
            value_or_none(record, "supplied_md5"),
            XSD.string,
        )
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["computed_md5"],
            value_or_none(record, "computed_md5"),
            XSD.string,
        )
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["viewer_type"],
            value_or_none(record, "viewer_type"),
            XSD.string,
        )
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["preview_state"],
            value_or_none(record, "preview_state"),
            XSD.string,
        )
        rdf.add(self.store, uri, rdf.DJHT["status"], value_or_none(record, "status"), XSD.string)
        rdf.add(
            self.store, uri, rdf.DJHT["upload_url"], value_or_none(record, "upload_url"), XSD.string
        )
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["upload_token"],
            value_or_none(record, "upload_token"),
            XSD.string,
        )
    ...
    def container_uri(self, item_id, item_type, account_id, account_uri):
        ...
        if uri is None:
            ...
            self.store.add(
                (uri, rdf.DJHT[f"{item_type}_id"], Literal(item_id, datatype=XSD.integer))
            )
    ...
    def insert_dataset(self, record):
        ...
        with self.lock_for_inserts:
            ...
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["url_public_html"],
                value_or_none(record, "url_public_html"),
                XSD.string,
            )
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["url_public_api"],
                value_or_none(record, "url_public_api"),
                XSD.string,
            )
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["url_private_html"],
                value_or_none(record, "url_private_html"),
                XSD.string,
            )
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["url_private_api"],
                value_or_none(record, "url_private_api"),
                XSD.string,
            )
            rdf.add(
                self.store,
            ...
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["defined_type"],
                value_or_none(record, "defined_type"),
                XSD.integer,
            )
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["defined_type_name"],
                value_or_none(record, "defined_type_name"),
                XSD.string,
            )
            rdf.add(
                self.store, uri, rdf.DJHT["citation"], value_or_none(record, "citation"), XSD.string
            ...
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["metadata_reason"],
                value_or_none(record, "metadata_reason"),
                XSD.string,
            )
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["confidential_reason"],
                value_or_none(record, "confidential_reason"),
                XSD.string,
            )
            rdf.add(
                self.store, uri, rdf.DJHT["funding"], value_or_none(record, "funding"), XSD.string
            )
            rdf.add(self.store, uri, rdf.DJHT["size"], value_or_none(record, "size"), XSD.integer)
            rdf.add(
                self.store, uri, rdf.DJHT["status"], value_or_none(record, "status"), XSD.string
            )
            rdf.add(
                self.store, uri, rdf.DJHT["version"], value_or_none(record, "version"), XSD.integer
            )
            ...
            rdf.add(
                self.store,
                uri,
                rdf.DJHT["figshare_url"],
                value_or_none(record, "figshare_url"),
                XSD.string,
            )
            rdf.add(
                self.store,
            ...
            rdf.add(
                self.store, uri, rdf.DJHT["is_metadata_record"], is_metadata_record, XSD.boolean
            )
            ...
            if is_embargoed:
                rdf.add(
                    self.store,
                    uri,
                    rdf.DJHT["embargo_until_date"],
                    value_or_none(record, "embargo_date"),
                    XSD.date,
                )
                rdf.add(
                    self.store,
                    ...
                    value_or_none(record, "embargo_type"),
                    XSD.string,
                )
                rdf.add(
                    self.store,
                    uri,
                    rdf.DJHT["embargo_title"],
                    value_or_none(record, "embargo_title"),
                    XSD.string,
                )
                rdf.add(
                    self.store,
                    uri,
                    rdf.DJHT["embargo_reason"],
                    value_or_none(record, "embargo_reason"),
                    XSD.string,
                )
            ...
            if review:
                logging.info(
                    "Adding review for dataset %s v%s.", uri, "0" if version is None else version
                )
                ...
                rdf.add(
                    self.store,
                    review_uri,
                    rdf.DJHT["request_date"],
                    value_or_none(review, "created_date"),
                    XSD.dateTime,
                )
                rdf.add(
                    self.store,
                    review_uri,
                    rdf.DJHT["modified_by_reviewer"],
                    value_or_none(review, "modified_date"),
                    XSD.dateTime,
                )
                rdf.add(
                    self.store, review_uri, rdf.DJHT["assigned_to"], assigned_to_uri, datatype="uri"
                )
    ...
    def insert_institution_group(self, record):
        ...
        rdf.add(
            ...
            uri,
            rdf.DJHT["parent_id"],
            value_or_none(record, "parent_id"),
        ...
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["association_criteria"],
            value_or_none(record, "association_criteria"),
            datatype=XSD.string,
        )
        rdf.add(
            self.store,
            uri,
            rdf.DJHT["is_featured"],
            value_or_none(record, "is_featured"),
    ...
    def insert_static_triplets(self):
        ...
        self.store.add(
            (rdf.DJHT["ReviewApproved"], RDFS.label, Literal("approved", datatype=XSD.string))
        )
        self.store.add((rdf.DJHT["ReviewRejected"], RDF.type, rdf.DJHT["ReviewType"]))
        self.store.add(
            (rdf.DJHT["ReviewRejected"], RDFS.label, Literal("rejected", datatype=XSD.string))
        )
        self.store.add((rdf.DJHT["ReviewClosed"], RDF.type, rdf.DJHT["ReviewType"]))
        self.store.add(
            (rdf.DJHT["ReviewClosed"], RDFS.label, Literal("closed", datatype=XSD.string))
        )
        ...
        self.store.add(
            (rdf.DJHT["ReviewAssigned"], RDFS.label, Literal("assigned", datatype=XSD.string))
        )
        self.store.add((rdf.DJHT["ReviewUnassigned"], RDF.type, rdf.DJHT["ReviewType"]))
        self.store.add(
            (rdf.DJHT["ReviewUnassigned"], RDFS.label, Literal("unassigned", datatype=XSD.string))
        )
        ...
        self.store.add(
            (rdf.DJHT["LogEntryDownload"], RDFS.label, Literal("download", datatype=XSD.string))
        )
        self.store.add((rdf.DJHT["LogEntryGitDownload"], RDF.type, rdf.DJHT["LogEntryType"]))
        self.store.add(
            (
                rdf.DJHT["LogEntryGitDownload"],
                RDFS.label,
                Literal("git_download", datatype=XSD.string),
            )
        )
        self.store.add((rdf.DJHT["LogEntryShare"], RDF.type, rdf.DJHT["LogEntryType"]))
        self.store.add(
            (rdf.DJHT["LogEntryShare"], RDFS.label, Literal("share", datatype=XSD.string))
        )
        ...
        self.store.add(
            (
                rdf.DJHT["LogEntryPrivateView"],
                RDFS.label,
                Literal("private_view", datatype=XSD.string),
            )
        )
        ...
        for language in languages:
            ...
            self.store.add((uri, RDFS.label, Literal(language["name"], datatype=XSD.string)))
            self.store.add(
                (uri, rdf.DJHT["shortcode"], Literal(language["shortcode"], datatype=XSD.string))
            )
    ...
    def mark_state_graph_as_initialized(self):
        ...
        """
        self.store.add(
            (URIRef("this"), rdf.DJHT["initialized"], Literal("true", datatype=XSD.boolean))
        )
