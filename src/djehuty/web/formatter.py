...
def format_codemeta_record(record, git_url, tags, authors, has_files, base_url):
    ...
    output = {
        ...
        "author": list(
            map(lambda author: format_codemeta_author_record(author, base_url), authors)
        ),
    }
    ...
    if has_files and "version" in record and "container_uuid" in record:
        download_url = [
            f"{base_url}/ndownloader/items/{record['container_uuid']}/versions/{record['version']}"
        ]
...
def format_rocrate_record(base_url, site_name, record, ror_url, tags, authors, files, git_url=None):
    ...
    if ror_url:
        publisher_records = [
            {"@id": ror_url, "@type": "Organization", "name": site_name, "url": base_url}
        ]
        dataset_record["publisher"] = {"@id": ror_url}

    return {
        "@context": "https://w3id.org/ro/crate/1.1/context",
        "@graph": [ro_crate_meta_record, dataset_record]
        + git_record
        + publisher_records
        + file_records,
    }
...
def format_dataset_record(record):
    ...
    if (
        bool(conv.value_or(record, "is_embargoed", False))
        and conv.value_or(record, "embargo_type", "") == "article"
    ):
        return {
...
def format_dataset_details_record(
    dataset, authors, files, custom_fields, tags, categories, funding, references, is_private=False
):
    ...
    if (
        (is_embargoed or is_restricted)
        and conv.value_or(dataset, "embargo_type", "") == "article"
        and not is_private
    ):
        return {
    ...
    return {
        ...
        "agreed_to_deposit_agreement": bool(
            conv.value_or_none(dataset, "agreed_to_deposit_agreement")
        ),
...
def format_collection_details_record(
    collection, funding, categories, references, tags, authors, custom_fields, datasets_count
):
    ...
    return {
        ...
        "timeline": {
            ...
            "publisherPublication": conv.value_or_none(
                collection, "timeline_publisher_publication"
            ),
        },
    }
...
def format_collection_version_record(record):
    ...
    urls = collection_urls(record)
    return {"version": version, "url": f"{urls['url_public_api']}/versions/{version}"}
        ...
        metadata = [
            {
                ...
                "value": {
                    "none": list(
                        map(lambda author: conv.value_or_none(author, "full_name"), authors)
                    )
                },
            }
        ]
