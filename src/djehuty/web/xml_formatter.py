...
def nlm(parameters):
    ...
    item = parameters["item"]
    maker.child(maker.child(meta, "title-group"), "article-title", {}, item["title"])
    ...
    maker.child(
        maker.child(meta, "pub-date", {"pub-type": "pub"}),
        "year",
        {},
        value_or(parameters, "published_year", None),
    )
...
def datacite_tree(parameters, debug=False):
    ...
    if "categories" in parameters:
        for cat in parameters["categories"]:
            maker.child(
                ...
                "subject",
                {
                    "subjectScheme": "Australian and New Zealand Standard Research "
    ...
    if has_organizations or has_contributors:
        ...
        if has_organizations:
            for name in parameters["organizations"]:
                contributor_element = maker.child(contributors_element, "contributor", type_att)
                maker.child(
                    contributor_element, "contributorName", {"nameType": "Organizational"}, name
                )

    # 09 dates
    if "published_date" in parameters:
        maker.child(
            maker.child(root, "dates"), "date", {"dateType": "Issued"}, parameters["published_date"]
        )
    ...
    if "fundings" in parameters:
        ...
        for funding in parameters["fundings"]:
            ...
            maker.child(
                funding_element, "funderName", {}, value_or(funding, "funder_name", "unknown")
            )
