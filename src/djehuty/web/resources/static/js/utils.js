...
function autocomplete_collaborator(event, item_id) {
    ...
    jQuery(".contributor-uuid").each(function () {
        existing_collaborators.push(jQuery(this).val());
    });
    if (current_text == "") {
        jQuery("#collaborator-ac").remove();
        jQuery("#add_collaborator").removeClass("input-for-ac");
...
function featured_groups(response) {
    jQuery.ajax({
        url: '/v3/groups',
        type: 'GET',
        contentType: 'application/json',
        ...
        const featured_groups = data.filter(group => group.is_featured === true);

        featured_groups.forEach(group => group_map.set(group.id, group));
        featured_groups.forEach(group => {
            if (group_map.has(group.parent_id)) return;
            const children = featured_groups.filter(child => child.parent_id === group.id);
            const ids = [group.id].concat(children.map(child => child.id));
        ...
        console.error("Failed to fetch featured groups");
        response([]);
    });
}
