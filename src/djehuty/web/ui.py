...
def read_saml_configuration(xml_root, logger):
    ...
    config.saml_config = {
        ...
        "idp": {
            ...
            "singleSignOnService": {"url": saml_idp_sso_url, "binding": saml_idp_sso_binding},
        ...
        "contactPerson": {
            "technical": {"givenName": "Technical support", "emailAddress": sp_tech_email},
            "support": {"givenName": "General support", "emailAddress": sp_support_email},
            "administrative": {
...
def read_configuration_file(server, config_file, logger, config_files):
    ...
    try:
        ...
        config.delay_inserting_log_entries = read_boolean_value(
            xml_root, "delay-inserting-log-entries", config.delay_inserting_log_entries, logger
        )
                    ...
                    if line == "---\n":
                        ...
                        if now_statement == line:
                            try:
                                ...
                                print(f"\rFailed to read '{timestamp_line}'.", file=sys.stderr)
...
def apply_transactions_from_directory(logger, server, transactions_directory):
    ...
    transactions = list(
        filter(
            lambda x: x.startswith("transaction_") and x.endswith(".sparql"), os.listdir(directory)
        )
    )
