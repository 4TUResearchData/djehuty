...
def order_direction(record, field_name, required=False, error_list=None):
    ...
    if value is not None and (not (value.lower() == "desc" or value.lower() == "asc")):
        return raise_or_return_error(
            error_list,
...
def integer_value(
    record, field_name, minimum_value=None, maximum_value=None, required=False, error_list=None
):
    """
...
def paging_to_offset_and_limit(record, error_list=None):
    ...
    if (page is not None or page_size is not None) and (offset is not None or limit is not None):
        return None, raise_or_return_error(
            error_list,
...
def __typed_value(
    record, field_name, expected_type=None, type_name=None, required=False, error_list=None
):
    """
...
def is_valid_uuid(value):
    ...
    return string_fits_pattern(
        value, 36, "^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$"
    )
...
def is_valid_url(value):
    ...
    return string_fits_pattern(
        value, 1024, "^(https?|ftps?)://[-a-zA-Z0-9@:%._\\+~#?&//=]{2,1024}$"
    )
