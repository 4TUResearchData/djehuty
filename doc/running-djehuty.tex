...
\section{Configuring CODECHECK requests}
  ...
  The option will be visible for the groups, for which \t{is\_featured} parameter is set to True.
...
\begin{tabularx}{\textwidth}{*{1}{!{\VRule[-1pt]}l}!{\VRule[-1pt]}X}
  ...
  \t{ror-url}                 & The ROR URL for this instance's organization.
                                Used in RO-Crate exports and as the ROR
                                link rendered next to the publisher when
                                the dataset's publisher matches \t{site-name}.\\
  \t{publisher-rors}          & Optional mapping of additional publisher
                                names to ROR URLs. Each child is a
                                \t{<publisher-ror name="..." url="..."/>}
                                element. The instance's own
                                (\t{site-name}, \t{ror-url}) pair is
                                auto-added.\\
