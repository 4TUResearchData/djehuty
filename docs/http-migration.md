...
- **Porting a group starts with two artifacts, made before writing any code:**
  1. **A side-effect catalog.** For each legacy handler in the group, including its
     private helpers, list every `db.*` write, cache invalidation, and e-mail send.
     The port must reproduce each row or consciously defer it with a note. Watch
     for calls to `db` methods that do not exist — nothing fails until the endpoint
     is actually exercised, so audit before porting, not after. The catalog is
     committed with the group PR as scaffolding and deleted when the group's legacy
     handlers are removed.
  2. **Consumer call-shape tests.** The legacy web UI is itself an API consumer
     with quirky call shapes (`limit=10000`, `remove_all` bodies, sub-resource
     POSTs). Grep the static JS for the group's routes and pin those exact shapes
     in the contract suite, verified green against legacy *before* the port exists —
     that is what proves the tests describe legacy rather than the port.
- Commits inside a PR are organized, not one blob. A typical shape:
  1. the side-effect catalog for the group
  2. extract framework-neutral logic into `djehuty.services`
  3. add the FastAPI router
  4. register the group + wire the dispatcher
  5. tests (unit + e2e)
- Before pushing, run the UI e2e groups that touch the ported routes against a
  default-`new` stack. The legacy pages drive the new endpoints through their
  JavaScript and catch what per-endpoint tests cannot: fields dropped by saves,
  and missing side effects that only surface several steps later.
