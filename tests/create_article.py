...
class TestDatasetFunctionality(unittest.TestCase):
    ...
    def test_crud_dataset(self):
        ...
        ## --------------------------------------------------------------------
        response = requests.delete(
            url=dataset_location, headers={"Authorization": f"token {TOKEN}"}, timeout=TIMEOUT
        )
    ...
    def test_search_dataset(self):
        ...
        response = requests.post(
            ...
            data=json.dumps(
                {"search_for": "Test", "limit": 15, "order": "title", "order_direction": "desc"}
            ),
    ...
    def test_non_existing_dataset(self):
        ...
        response = requests.get(
            url=f"{BASE_URL}datasets/1234567890", headers={"Accept": "text/html"}, timeout=TIMEOUT
        )
