...
def test_codemeta_passes_git_url_to_the_formatter(monkeypatch):
    ...
    monkeypatch.setattr("djehuty.api.v3.codemeta.formatter.format_codemeta_record", _fake_format)
...
def test_dataset_tags_add_reads_full_existing_list():
    ...
    db = _TagsDB()
    client = TestClient(create_app(db))
    resp = client.post(f"/v3/datasets/{_DATASET_UUID}/tags", json={"tags": ["new"]}, headers=_AUTH)
    assert resp.status_code == 205
    _, _, kwargs = _last_call(db, "tags")
    assert kwargs.get("limit") == 10000


def test_dataset_tags_delete_absent_tag_is_500():
    # AS-IS: legacy 500s (error_500, empty body) when the tag isn't present, so
    # the port does too rather than answering a 204 success. The full list is
    # still read first (limit=10000) so a real removal never truncates it.
    db = _TagsDB()
    client = TestClient(create_app(db))
    resp = client.delete(
